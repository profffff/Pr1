...
@njit(parallel=True)
def sinc_paper_optimized(X_t, A, N_used, TH, max_EVM, Nfft, Ndac, Nant, SINC_t):
    ...
    SINC_mtx = np.zeros((Nfft, Nfft), dtype=np.complex128)
    for j in range(Nfft):
        SINC_mtx[j, :] = np.roll(SINC_t, j)

    S_t_canc = np.zeros((Nsym, Nant, Nfft), dtype=np.complex128)
    S_t_ant_new = np.zeros((Nsym, Nant, Nfft), dtype=np.complex128)
    S_t_ant_canc_peak = np.zeros((Nsym, Nant, Nfft), dtype=np.complex128)

    M_iter = 2  # Number of iterations for peaks positions search per interval

    # find peaks to reduce
    for i1 in prange(Nsym):
        for i2 in range(Nant):
            S_t = S_t_ant[i1, i2, :].copy()

            # PAPR reduction algorithm
            for j in range(N_iter):
                S_t_canc_tmp = np.zeros(Nfft, dtype=np.complex128)

                for p in range(M_iter):
                    Nintervals = Nfft // N_samples

                    # Peaks positions and amplitude per interval
                    sinc_Ampl = np.zeros(Nintervals, dtype=np.complex128)
    ...
    S_t_dac_canc_sig = np.zeros((Nsym, Ndac, Nfft), dtype=np.complex128)
    S_t_dac_canc_peak = np.zeros((Nsym, Ndac, Nfft), dtype=np.complex128)
    S_t_ant_canc_peak_new = np.zeros((Nsym, Nant, Nfft), dtype=np.complex128)
    ...
    for i1 in prange(Nsym):
        for i3 in range(Nfft):
            ...
            if indx.size > 0:
                A_new = A[indx, :]
                sig_new = sig[indx]
                s1 = np.linalg.pinv(A_new) @ np.ascontiguousarray(sig_new)   # peaks only transform
                s2 = np.linalg.pinv(A) @  np.ascontiguousarray(sig)  # peaks + zeros transform
                S_t_dac_canc_peak[i1, :, i3] = coef1 * s1 + coef2 * s2
            else:
                S_t_dac_canc_peak[i1, :, i3] = np.zeros(Ndac)

        # Go back to antenna domain
        S_t_ant_canc_peak_new[i1, :, :] = A @ np.ascontiguousarray(S_t_dac_canc_peak[i1, :, :])

        # Convolve peaks with sinc functions in DAC domain to satisfy spectrum mask
        for i2 in range(Ndac):
            s_t = S_t_dac_canc_peak[i1, i2, :]
            S_t_dac_canc_sig[i1, i2, :] = s_t @ np.ascontiguousarray(SINC_mtx)
    ...
    S_t_ant_new2 = np.zeros((Nsym, Nant, Nfft), dtype=np.complex128)
    for i1 in range(Nsym):
        S_t_ant_new2[i1, :, :] = A @ np.ascontiguousarray(S_t_dac_canc_sig[i1, :, :])
...
def precompute_sinc(Nfft, N_used):
...
def sinc_paper_parallel(X_t, A, N_used, TH, max_EVM, Nfft, scen):
    Ndac = scen['Ndac']
    Nant = scen['Ntx']
    
    # Предварительный расчёт SINC_t
    SINC_t = precompute_sinc(Nfft, N_used)
    
    # Вызов оптимизированной функции
    return sinc_paper_optimized(X_t, A, N_used, TH, max_EVM, Nfft, Ndac, Nant, SINC_t)
